  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-4">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <h3 className="text-xl md:text-2xl font-bold text-slate-800">挂号列表</h3>
        <button 
          onClick={() => setShowModal(true)}
          className="w-full md:w-auto bg-blue-600 text-white px-6 py-2.5 rounded-xl font-semibold shadow-md hover:bg-blue-700 transition-all flex items-center justify-center gap-2"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: '今日就诊率', value: `${Math.round((patients.filter(p => p.status === '已完成').length / (patients.length || 1)) * 100)}%`, icon: 'fa-chart-pie', color: 'bg-indigo-500' },
        ].map((stat, i) => (
          <div key={i} className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center space-x-4">
            <div className={`${stat.color} w-10 h-10 md:w-12 md:h-12 rounded-xl flex items-center justify-center text-white shadow-lg shrink-0`}>
            ...
            <tbody className="divide-y divide-slate-100">
              {patients.map((p) => (
                <tr key={p.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-50">{p.registerTime}</td>
                  ...
                  <td className="px-6 py-4">
                    <button className="text-blue-600 hover:text-blue-800 font-medium text-sm">打印</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-end md:items-center justify-center z-50 p-0 md:p-4">
          <div className="bg-white rounded-t-2xl md:rounded-2xl w-full max-w-md p-6 md:p-8 shadow-2xl animate-slideUp md:animate-zoomIn">
            ...
            <div className="mt-8 flex gap-3">
              ...
              <button 
                ...
                className="flex-1 px-4 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 font-bold shadow-sm"
              >
                确认挂号
              </button>
            </div>
